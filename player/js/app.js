// 全局变量
let currentApiSource = localStorage.getItem('currentApiSource') || '';// '' 中不填写代表：聚合搜索
...
async function search() {
    ...
    try {
        ...
        // 添加XSS保护，使用textContent和属性转义，来源tag样式
        resultsDiv.innerHTML = results.map(item => {
            ...
            const sourceInfo = item.source_name ? 
                `<span class="bg-[#bde4f6] text-xs px-2 py-1 rounded-full">${item.source_name}</span>` : '';
            ...
            modalContent.innerHTML = `
                <div class="flex justify-end mb-2">
                    <button onclick="toggleEpisodeOrder()" class="px-4 py-2 bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 text-white font-semibold rounded-full shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 flex items-center justify-center space-x-2">
                        ...
                        <span style="color:#2c3e50;">倒序排列</span>
                    </button>
                </div>
...
function renderEpisodes(vodName) {
    ...
    return episodes.map((episode, index) => {
        // 根据倒序状态计算真实的剧集索引，集数模态框样式
        const realIndex = episodesReversed ? currentEpisodes.length - 1 - index : index;
        return `
            <button id="episode-${realIndex}" onclick="playVideo('${episode}','${vodName.replace(/"/g, '&quot;')}', ${realIndex})" 
                    class="px-4 py-2 text-[#2c3e50] bg-[#222] hover:bg-[#333] border border-[#333] rounded-lg transition-colors text-center episode-btn">
                第${realIndex + 1}集
            </button>
        `;
    }).join('');
